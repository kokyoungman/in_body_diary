...
class MainManager {
	...
	getRemainDateText = (targetDate, nowDate) => {
		...
		if (remainOnlyDay > 0) {
			...
			const beforeMonthDay = new Date(nowDate.getFullYear(), nowDate.getMonth(), 0).getDate();
	...
	showUserBaseData = () => {
		...
		if (this.userSelect.selectedIndex == 0) {
			...
			this.numbers[0].forEach((number, index) => {
				const weight = this.gapValues[this.userSelect.selectedIndex - 1].weight[index];
				number.textContent = weight + ` (${this.getBmi(this.userHeight, weight)})`;
			});
			this.numbers[1].forEach((number, index) => {
				number.textContent = this.gapValues[this.userSelect.selectedIndex - 1].bodyFat[index];
			});
			this.numbers[2].forEach((number, index) => {
				number.textContent = this.gapValues[this.userSelect.selectedIndex - 1].water[index];
			});
			this.numbers[3].forEach((number, index) => {
				number.textContent = this.gapValues[this.userSelect.selectedIndex - 1].muscle[index];
			});
			this.numbers[4].forEach((number, index) => {
				number.textContent = this.gapValues[this.userSelect.selectedIndex - 1].bone[index];
			});
			this.numbers[5].forEach((number, index) => {
				number.textContent = this.gapValues[this.userSelect.selectedIndex - 1].visceralFat[index];
			});
			this.numbers[6].forEach((number, index) => {
				number.textContent = this.gapValues[this.userSelect.selectedIndex - 1].calorie[index];
			});

			this.gradeTexts[4].forEach((gradeText, index) => {
				gradeText.textContent = this.gradeTextValues[this.userSelect.selectedIndex - 1].bone[index] + "kg 미만이면, 건강";
			});
		}
	...
	addHistory = () => {
		...
		if (saveDatas !== null) {
			...
			parsedSaveDatas.forEach((saveData) => {
				if (saveData[0] == this.inputs[0].value &&
		...
		if (isData == false) {
			for (let i = parsedSaveDatas.length - 1; i >= 0; i--) {
				if (parsedSaveDatas[i][0] == this.inputs[0].value &&
		...
		localStorage.setItem(this.userSelect.selectedIndex, JSON.stringify(parsedSaveDatas));
	}
		...
		if (saveDatas !== null) {
			...
			parsedSaveDatas.forEach((saveData, dayIndex) => {
				...
				saveData.forEach((saveDataValue, itemIndex) => {
					...
					if (0 <= dayIndex && dayIndex < this.viewDataCount && 0 <= itemIndex) {
						let targetGapValues;

						if (itemIndex == 0)
							targetGapValues = this.gapValues[this.userSelect.selectedIndex - 1].weight;
						else if (itemIndex == 1)
							targetGapValues = this.gapValues[this.userSelect.selectedIndex - 1].bodyFat;
						else if (itemIndex == 2)
							targetGapValues = this.gapValues[this.userSelect.selectedIndex - 1].water;
						else if (itemIndex == 3)
							targetGapValues = this.gapValues[this.userSelect.selectedIndex - 1].muscle;
						else if (itemIndex == 4)
							targetGapValues = this.gapValues[this.userSelect.selectedIndex - 1].bone;
						else if (itemIndex == 5)
							targetGapValues = this.gapValues[this.userSelect.selectedIndex - 1].visceralFat;
						else if (itemIndex == 6)
							targetGapValues = this.gapValues[this.userSelect.selectedIndex - 1].calorie;
						...
						const maxValue = targetGapValues.find((value, index) => {
							...
							if (targetGapValues.length - 1 == index)
								return true;

							return saveDataValue < value;
						});

						if (itemIndex == 0 || itemIndex == 1 || itemIndex == 2 || itemIndex == 3 || itemIndex == 4)
							this.arrows[dayIndex][itemIndex].textContent = "🥕" + parseFloat(saveDataValue).toFixed(1);
						else
							this.arrows[dayIndex][itemIndex].textContent = "🥕" + saveDataValue;
						...
						if (dayIndex == 0)
							...
							this.arrows[dayIndex][itemIndex].style.backgroundColor = "#F1FE2D";

						this.arrows[dayIndex][itemIndex].style.fontSize = String(20 - dayIndex * 4) + "px";
						...
						if (minValue == "-")
							...
							if (minIndex == 0 && saveDataValue <= minValue)
								this.arrows[dayIndex][itemIndex].style.left = "0%";
							else if (minIndex == targetGapValues.length - 2 && saveDataValue >= maxValue)
								this.arrows[dayIndex][itemIndex].style.left = "100%";
							else {
	...
	showHistoryForChart = () => {
		...
		if (saveDatas !== null) {
			...
			parsedSaveDatas.forEach((saveData, dayIndex) => {
				...
				saveData.forEach((saveDataValue, itemIndex) => {
					...
					if (0 <= dayIndex && dayIndex < this.viewDataCount && 0 <= itemIndex) {
						...
						{
							if (remainDay == 0)
								weightLabels.unshift("오늘");
							else
								weightLabels.unshift(this.getRemainDateText(targetDate, nowDate));
			...
			worldWeightChart.change(weightLabels, weightValues, weightKgs, worldWeightGradeValues, worldWeightGradeKgs);
			koreaWeightChart.change(weightLabels, weightValues, weightKgs, koreaWeightGradeValues, koreaWeightGradeKgs);
		}
	}
