// 자바스크립트 프로젝트 구현 - 바디 뷰어 (conditional_gaussian.js)
...
var ConditionalMultivariateGaussian = Class.create({
  ...
  set_index_partition: function () {
    var index, value;
    var u = this.number_of_unconditioned_variables,
  ...
  condition_on_indices: function (
    newly_conditioned_indices,
    ...
    if (partition_changed) {
      ...
      this.sigma_22_inverse_times_offsets = matrix_times_vector(
  ...
  set_sigma_22: function () {
    ...
    for (i = this.number_of_conditioned_variables; i; ) {
      ...
      for (j = this.number_of_conditioned_variables; j; ) {
        --j;
  ...
  update_conditioned_values: function (
  ...
  offset_vector_for_column: function (column_index) {
    var sigma_22_column_for_index =
      this.sigma_22_inverse_tranpose[column_index];
    var vector = new Array(this.max_index);

    for (var j = this.number_of_unconditioned_variables; j; ) {
